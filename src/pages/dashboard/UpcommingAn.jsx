...
import { soonAnime } from "../../service/server";
import { useNavigate } from "react-router-dom";

export default function UpcommingAn() {
    ...
    const fetchAnime = async (retryCount = 0) => {
        try {
            const data = await soonAnime();
            setAnime(data.data || []);
        } catch (err) {
            if (err.response?.status === 429) {
                if (retryCount < 5) { // avoid infinite loop
                    console.warn("Too many requests, retrying in 1 sec...");
                    setTimeout(() => fetchAnime(retryCount + 1), 1000);
                } else {
                    console.error("Max retries reached for soonAnime.");
                }
            } else {
                console.error(err);
            }
