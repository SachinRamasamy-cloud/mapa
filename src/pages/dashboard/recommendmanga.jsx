...
import { recommendManga } from "../../service/server";
import { useNavigate } from "react-router-dom";

export default function RecommendedMangaSection() {
    ...
    const fetchManga = async (retryCount = 0) => {
        try {
            ...
            // Map recommended entry to actual manga object
            const cleanData = data.data?.map(item => item.entry[0]) || [];
            setManga(cleanData);
            setLoading(false);
        } catch (err) {
    ...
    if (loading)
        return (
            <p className="text-xl text-black dark:text-white px-6 mt-6">
                Loading recommended manga...
            </p>
        );
        ...
        <div className="mt-4 relative">
            ...
            <div className="flex justify-between items-center px-6 py-3">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <button
                        onClick={scrollLeft}
                        className="hidden lg:block text-black dark:text-white bg-white/50 dark:bg-gray-700 hover:bg-white/80 dark:hover:bg-gray-600 p-2 rounded-full shadow-lg"
                    >
                        &#10094;
                    </button>
